...
```{r Output Directory}
outputdir <- paste0('/multiomics/results/figures/survival_analysis/DE/',params$comparison,'/')
...
data <- list(proteomics=read_tsv(file = files[str_detect(files,'full')]),
             phosphoproteomics=read_tsv(file = files[str_detect(files,'phospho')]),
             transcriptomics=read_tsv(file = files[str_detect(files,'transcript')]))

idCol <- list(proteomics='Protein.Names',phosphoproteomics='Protein.Names',transcriptomics='Gene.ID')
omics_type <- list('_proteomics','_phosphoproteomics','')
```
