...
design <- read_tsv('~/multiomics/results/design_files/design_multiomics_mofa.tsv') %>% 
  filter(str_detect(InvCa.type,'Ductal'))
...
finalData <- list(trainData=dataTrain,responseTrain=responseTrain,testData=dataTest,responseTest=responseTest)
save(finalData,file = '~/multiomics/results/DIABLO/LNposvsLNneg/Ductal/modelData.gzip',compress = 'gzip')
...
perf.diablo <-  perf(basic.diablo.model, validation = 'Mfold', 
                   folds = 10, nrepeat = 10,signif.threshold = 0.05,progressBar = TRUE,auc = TRUE,cpus = (parallel::detectCores()-2)) 
...
saveRDS(object = final.diablo.model,file = '~/multiomics/results/DIABLO/LNposvsLNneg/Ductal/model.gzip',compress = 'gzip')
```

## Table of selected variables
```{r}
ncomp <- final.diablo.model$ncomp[['Y']]
modelVar <- map(.x = 1:ncomp,.f = function(x){selectVar(final.diablo.model,comp=x)})
...
write_tsv(x = modelVar,file = '~/multiomics/results/DIABLO/LNposvsLNneg/Ductal/model_variables.tsv')
```

## Figures
### Market Plot
```{r,message=FALSE}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/marker_plot.pdf',width = 30,height = 10,onefile = TRUE)
...
### DIABLO Plot
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/corrplot.pdf',width = 10,height = 10,onefile = TRUE)
...
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/indiv_plot.pdf',width = 10,height = 10,onefile = TRUE)
...
### Circos Plot
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/circos_plot.pdf',onefile = TRUE,width = 8.5)
...
### Network Plot
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/network_plot.pdf',onefile = TRUE,width = 20,height = 20)
...
### Loadings Plot
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/loadings_plot.pdf',onefile = TRUE,width = 20,height = 20)
...
### Heatmap
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/heatmap.pdf',onefile = TRUE,width = 10,height = 10)
...
### Plot AUC
```{r}
pdf(file = '~/multiomics/results/figures/DIABLO/LNposvsLNneg/Ductal/roc_curve.pdf',onefile = TRUE,width = 10,height = 10)
...
perf.diablo <- perf(final.diablo.model, validation = 'Mfold',folds = 10,nrepeat = 10,auc = TRUE,signif.threshold = 0.05,
