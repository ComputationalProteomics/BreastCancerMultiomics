...
engine: knitr
params:
  outputdir: '~/multiomics/results/figures/survival_analysis/MOFA/DuctalvsLobular/'
  model: '~/multiomics/results/MOFA/all_samples/infiltration/model.RDS'
  factor: 3
---
...
fit <- coxph(formula = coxFormula,data = factors)
summary(fit)
```
```{r}
#| tbl-column: page
...
featList <- list(proteomics={featFactors %>%dplyr::filter(view=='proteomics',factor==paste0('Factor',params$factor)) %>% select(feature) %>% unlist %>% as.character %>% unique},
              ...
              infiltration={featFactors %>% dplyr::filter(view=='infiltration',factor==paste0('Factor',params$factor)) %>% select(feature) %>% unlist %>% as.character %>% unique}) 
...
immuneFeatures <- get_data(object = model,features = list(infiltration=featList$infiltration),as.data.frame = TRUE) %>% pivot_wider(id_cols = 'sample',names_from = feature,values_from = value)

finalFeatures <- reduce(.x = list(protFeatures,phosphoFeatures,rnaFeatures,immuneFeatures),.f = ~left_join(x = .x,y = .y,by = 'sample'))
...
fit2 <- coxph(formula = coxFormula,data = finalFeatures)
...
finalFeaturesFactors <- map(.x = variables,.f = function(x){(finalFeatures[[x]]>featCut[["cutpoint"]][x,1])})
names(finalFeaturesFactors) <- variables
finalFeaturesFactors <- as.data.frame(finalFeaturesFactors) %>% 
