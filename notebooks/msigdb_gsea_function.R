...
#     dplyr::select(genecol,signedPVal) %>% 
#     dplyr::filter(!is.na(signedPVal)) %>% 
#     dplyr::filter(!is.na(!!sym(genecol))) %>% 
#     dplyr::group_by(.[,genecol]) %>% 
#     group_nest()
#   genelist <- purrr::map(data$data,~median(.x$signedPVal,na.rm=T)) %>% unlist
#   names(genelist) <- data %>% dplyr::pull(genecol)
...
#   term2gene <- msigdbr(species = species,collection = collection) %>% dplyr::select(gs_name,gene_symbol)
...
#   term2gene <- msigdbr(species = species,collection = collection) %>% dplyr::select(gs_name,human_ensembl_gene)
...
#   term2gene <- msigdbr::msigdbr(species = 'Homo sapiens',collection = 'H') %>% dplyr::select(gs_name,gene_symbol)
#   term2geneRNA <- msigdbr::msigdbr(species = 'Homo sapiens',collection = 'H') %>% dplyr::select(gs_name,gene_symbol)
