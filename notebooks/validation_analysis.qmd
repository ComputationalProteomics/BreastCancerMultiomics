...
```{r Output Directory}
outputdir <- paste0('/home/multiomics/results/figures/validation/',params$comparison,'_',params$event,'/')
...
```{r}
design <- read.xlsx(xlsxFile = 'https://static-content.springer.com/esm/art%3A10.1038%2Fs41523-022-00465-3/MediaObjects/41523_2022_465_MOESM2_ESM.xlsx',sheet = 1)
design_study <- read_tsv(file = '/home/multiomics/results/design_files/mergedDesign_ref_hisscore.tsv')
...
data_all <- SCANB.9206.genematrix_noNeg |> as.data.frame() |> rownames_to_column(var='Gene')
# newGenes <- data_all |> 
#   dplyr::select(sampleFilter[["GEX.assay"]])

# newGenes <- newGenes %>% 
#   performCyclicLoessNormalization(noLogTransform = FALSE)

# data_all <- bind_cols(Gene = data_all[["Gene"]], newGenes)

data_filter <- data_all %>% select(Gene,sampleFilter$GEX.assay)
...
coxFeats <- list.files(path = '/home/multiomics/results/figures/survival_analysis/',pattern = 'cox_analysis_res',full.names = TRUE,recursive = TRUE) %>% 
...
idMap <- list.files(path = "/home/multiomics/results/",pattern = "featIDs\\.tsv",full.names = TRUE,recursive = TRUE) %>% 
  .[str_detect(.,params$comparison)] %>% 
  map(function(x){read_tsv(file = x,col_types = colTypes)}) %>% 
  list_rbind() %>% 
  filter(!duplicated(.)) 
