...
data <- pmap(list(data,idCol,omics_type),function(x,y,z){
  ...
  data <- x %>% 
  select(all_of(y),all_of(genecol),all_of(signifCol),design$sample_id) %>% 
  filter(.[[signifCol]]<0.1)
})

featIds <- map(data,function(x){
  data <- x %>% 
    select(matches("Name|Gene|Ids|Sequence|Phosphosite|Residue"))
}) %>% 
  list_rbind()

write_tsv(x = featIds,file = paste0(outputdir,'featIDs.tsv'))
...
if(!is_empty(signif_omics)){
  ...
  ggsave(filename = 'forestplot_univariable_DE.pdf',plot = uniplot,device = 'pdf',path = outputdir,dpi = 600,width = 9,height = 9,units = 'in')
}else(
...
uniTable <- forest_model(model_list = modelList,return_data = TRUE) %>% 
  ...
  left_join(y = featIds,by = c("variable"="Protein.Names")) %>% 
  relocate(c(12:20),.after = "variable")
