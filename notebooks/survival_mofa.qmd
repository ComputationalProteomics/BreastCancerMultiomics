...
engine: knitr
params:
  model: '/home/multiomics/results/MOFA/AllSamples_noG2/model.RDS'
  outcome: 'LN'
  event: 'RFi_event'
  time: 'RFi_days'
---
...
## MOFA Factors
```{r}
outputdir <- str_split(string = params$model,pattern = '/',n = 5)[[1]][5] %>% 
  str_split_1(pattern = '/') %>% 
  .[1:length(.)-1] %>% 
  paste0(.,collapse = '/')
outputdir <- paste0('/home/multiomics/results/figures/survival_analysis/',outputdir,'/',str_split_i(outputdir,'/',2),'_',params$outcome,'_',params$event,'/')
...
if(!is_empty(signif_omics)){
  ...
  ggsave(filename = 'forestplot_univariable_MOFA.pdf',plot = uniplot,device = 'pdf',path = outputdir,dpi = 600,width = 9,height = 9,units = 'in')
}
...
featIds <- params$model %>% 
  str_split_i('model.RDS',1) %>% 
  list.files(pattern = 'featIDs',full.names = TRUE,recursive = TRUE) %>% 
  read_tsv()

uniTable <- uniTable %>% 
  left_join(y = featIds, by= c("variable"="Unique.Ids")) %>% 
  relocate(c(12:21),.after="variable")
```
...
conditionally_eval <- function() {
  if (is_empty(signif_omics)) {
    ...
    writeLines(text = 'No variables were deemed significant (p-value < 0.05) at univariable model',con = paste0(outputdir,'error.txt'))
  }
}
...
signif_omics <- map(.x = seq_along(cox_models_omics_multivariate),function(x){
  ...
  list_rbind() %>% filter(`Pr(>|z|)`<0.05) %>% select(Feature) %>% as_vector()
