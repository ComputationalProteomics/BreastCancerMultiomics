...
clinical <- design %>% 
  ...
  mutate(Age=factor(Age,levels=c('<50','≥50')),
         NHG=factor(NHG,levels=c('1|2','3')),
         Size=factor(Size,levels=c('0','1'),labels=c('<20mm','≥20mm')),
         Treatment=factor(Treatment,levels=c('Chemo','ChemoEndo','Endo','None'))) %>% 
  drop_na()
...
basic_recipe <- recipe(x =  data) %>%
  update_role(x,all_of(clinical_variables),new_role = 'predictor') %>%
  update_role(sample_id,new_role = 'ID') %>%
  update_role(surv, new_role = 'outcome') #%>% 
  # step_string2factor(all_string_predictors()) %>% 
  # step_dummy(all_factor_predictors()) %>% 
...
#   plotPH <- ggforest(model = fit,data = finalFeaturesProcessed)
#   list(fitcheck,plotPH)
# })
