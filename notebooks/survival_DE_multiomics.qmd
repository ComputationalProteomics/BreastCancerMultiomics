...
clinical <- design %>% 
  ...
  mutate(across(contains('HisScore'),~case_when(.x>=50~'High',
                                                between(.x,10,50)~'Intermediate',
                                                .x<10~'Low'))) %>% 
  drop_na()
...
lassoPlot <- cv.fit[["glmnet.fit"]] %>% autoplot(best_penalty = get(params$lambda),top_n = 5)
...
featSparse <- coef(cv.fit,s = get(params$lambda)) %>% as.matrix() %>% as.data.frame() %>% filter(`1`!=0) %>% arrange(desc(abs(`1`)))
...
while(!is.null(wf[["warning"]])){
lambda <- get(params$lambda)+(get(params$lambda)*counter)
featSparse <- coef(cv.fit,s = lambda) %>% as.matrix() %>% as.data.frame() %>% filter(`1`!=0) %>% arrange(desc(abs(`1`)))
counter <- counter+1
...
```{r}
##### Check fit
fitcheck <- cox.zph(fit = extract_fit_engine(wf[["result"]]))
...
#### Plot Hazard Ratios
```{r}
plotPH <- ggforest(model = extract_fit_engine(wf[["result"]]),data = dataPreprocess)
...
```{r}
selectedFeat[which(!(selectedFeat %in% colnames(finalFeatures)))] <- str_remove_all(string = selectedFeat[which(!(selectedFeat %in% colnames(finalFeatures)))],pattern = '_[^_]*$')
...
featCutMedian <- finalFeatures %>% 
  ...
  bind_cols(finalFeatures[,c('sample_id','time','event')],.) %>% 
  mutate(surv=Surv(time=time,event=event),!!params$outcome:=finalFeatures[[params$outcome]])
```
...
kmPlots <- map(.x = selectedFeat,.f = function(x){
  ...
  ggsurvplot(fit = fit,data = featCutMedian,conf.int = FALSE,pval = TRUE,pval.method = TRUE,surv.median.line = 'hv',risk.table = TRUE)
  })
```
```{r}
invisible(pdf(file = paste0(outputdir,'kaplan_meier_lasso.pdf'),width = 20,height = 10,onefile = TRUE))
invisible(map(1:length(kmPlots),~print(kmPlots[[.x]])))
invisible(dev.off())

varKeep <- list('params','varKeep','catchWarning')
