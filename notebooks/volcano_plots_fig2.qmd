...
```{r Output Directory}
outputdir <- '/home/multiomics/results/figures/volcano_plots/'
...
```{r}
prot <- read_tsv(file = '/home/multiomics/results/differentialexpression/DuctalvsLobular/ductalvslobular_fullproteome_protein/ductalvslobular_fullproteome_protein_stats.tsv')
phospho <- read_tsv(file = '/home/multiomics/results/differentialexpression/DuctalvsLobular/ductalvslobular_phosphoproteome_peptide_aggModSeq/ductalvslobular_phosphoproteome_peptide_aggModSeq_stats.tsv')
gene <- read_tsv(file = '/home/multiomics/results/differentialexpression/DuctalvsLobular/ductalvslobular_transcriptome/ductalvslobular_transcriptome_stats.tsv')
```
...
phospho_clean <- phospho %>% 
  ...
  mutate(Expression = case_when(
    ...
    mutate(Feature = str_c(Protein.Names, Phosphosite, Residue, sep = "."))
...
plot_volcano_prot <- prot_clean %>% 
  ...
  geom_text_repel(data = prot_significant, mapping = aes(x = Log2FC,y = -log10(FDR),colour = Expression, label = Protein.Names),size=4, max.overlaps = 5)
...
plot_volcano_phospho <- phospho_clean %>% 
  ...
  labs(tag = 'B') +
  theme_bw(base_size = 20) +
  geom_text_repel(data = phospho_significant, mapping = aes(x = Log2FC,y = -log10(FDR),colour = Expression, label = Feature),size=4,max.overlaps = 5)
...
plot_volcano_gene <- gene_clean %>% 
  ...
  labs(tag = 'C') +
  theme_bw(base_size = 20) +
  geom_text_repel(data = gene_significant, mapping = aes(x = Log2FC,y = -log10(FDR),colour = Expression, label = Gene.Name),size=4)
