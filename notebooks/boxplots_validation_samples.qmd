...
```{r}
#Output Directory
outputdir <- paste0("/multiomics/results/figures/survival_analysis/MOFA/",params$comparison,"/",params$comparison,"_",params$outcome,"_",params$event,"/")
colTypes <- cols("Residue"=col_character(),.default = col_guess())

coxFeats <- list.files(path = '/multiomics/results/figures/survival_analysis/MOFA/',pattern = 'cox_analysis_res',full.names = TRUE,recursive = TRUE) %>% 
  .[str_detect(.,params$comparison)] %>% 
  .[str_detect(.,params$event)] %>% 
  map(function(x){read_tsv(file = x,col_types = colTypes)}) %>% 
  list_rbind() %>% 
