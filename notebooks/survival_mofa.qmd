...
## MOFA Factors
```{r}
outputdir <- paste0('/multiomics/results/figures/survival_analysis/MOFA/',str_split_i(string = params$model,pattern = '/',i = 5),'/')
...
features <- MOFA2::get_weights(object = model,views = 'all',factors = c(params$factor),as.data.frame = TRUE,scale = TRUE)
...
signif_omics <- map(.x = seq_len(length(cox_models_omics)),function(x){
  pluck(cox_models_omics[[x]],'coefficients') %>% as.data.frame() %>% filter(.[['Pr(>|z|)']]<0.1) %>% rownames(.)
}) %>% 
  as_vector() %>% 
  unique

signif_clinical <- map(.x = seq_len(length(cox_models_clinical)),function(x){
  pluck(cox_models_clinical[[x]],'coefficients') %>% as.data.frame() %>% filter(.[['Pr(>|z|)']]<0.1) %>% rownames(.)
}) %>% 
  as_vector() %>% 
  unique
...
finalFeaturesFactors <- surv_categorize(x = featCut,labels = c('Low','High')) %>% as_tibble() %>% mutate(surv=Surv(time=time,event=event),LN=finalFeatures$LN,PR=finalFeatures$PR)

featCutMedian <- finalFeatures %>% mutate(across(c(signif_omics),~if_else(.x<median(.x,na.rm=TRUE),true = 'Low',false = 'High'))) %>% mutate(surv=Surv(time=time,event=event),LN=finalFeatures$LN,PR=finalFeatures$PR)
```
...
finalFeaturesFactors <- surv_categorize(x = featCut,labels = c('Low','High')) %>% as_tibble() %>% mutate(surv=Surv(time=time,event=event),group=finalFeatures[['group']])
