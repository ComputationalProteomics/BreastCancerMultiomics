...
signif_omics <- map(.x = seq_len(length(cox_models_omics)),function(x){
  pluck(cox_models_omics[[x]],'coefficients') %>% as.data.frame()
}) %>% 
  list_rbind() %>% 
  mutate(PValue.Adj=p.adjust(.[['Pr(>|z|)']],method = 'fdr')) %>% 
  filter(PValue.Adj<0.1) %>% 
  rownames() %>% 
  as_vector() %>% 
  unique

signif_clinical <- map(.x = seq_len(length(cox_models_clinical)),function(x){
  pluck(cox_models_clinical[[x]],'coefficients') %>% as.data.frame() 
}) %>% 
  list_rbind() %>% 
  mutate(PValue.Adj=p.adjust(.[['Pr(>|z|)']],method = 'fdr')) %>% 
  filter(PValue.Adj<0.1) %>% 
...
featCutMedian <- finalFeatures %>% mutate(across(all_of(signif_omics),~if_else(.x<median(.x,na.rm=TRUE),true = 'Low',false = 'High'))) %>% mutate(surv=Surv(time=time,event=event),!!params$outcome:=finalFeatures[[params$outcome]])
```
