...
```{r}
if(!dir.exists(paths = '/home/multiomics/results/phospho_aggregation/')) {
  dir.create(path = '/home/multiomics/results/phospho_aggregation/',recursive = TRUE)
}
outputdir <- '/home/multiomics/results/phospho_aggregation/'
```

```{r}
library("Biostrings")

fastaFile <- readAAStringSet("/home/multiomics/data/Sergio_mergedb.fasta")
...
input_path <- '/home/multiomics/results/normalization/phosphoproteome/peptide_normalization/'
...
#  print (paste(x[1],x[2],x[3],offset,pos,protein))
  if (offset>-1 && pos>-1) {
    ...
    if (length(offsets)>1) {
      ...
      residue=paste(residue, str_sub(cleanSeq,offset2-1,offset2-1), sep=",")
      nphos=2
...
```{r}
designP <- "/home/multiomics/results/design_files/design_phospho_g1g2_g1g34.tsv"
design <- read_tsv("/home/multiomics/results/design_files/design_phospho_ductal.tsv")
design <- design %>% 
  ...
  select(Sample,Group.Info,ms_run_order)
...
normalyzerDE(jDEName, designPath = designP, dataPath = agg_file, comparisons = comparisons, sigThres = 0.1, sampleCol = "Sample", condCol = "Group.Info", outputDir = outputdir, leastRepCount = 3, batchCol = 'ms_run_order')
...
gctFile <- paste0(outputdir,"comparisons.gct")

statstable <- read_tsv(paste0(outputdir,jDEName,"/",jDEName,"_stats.tsv"))
...
# Download database
```{r}
download.file(url = "https://raw.githubusercontent.com/broadinstitute/ssGSEA2.0/refs/heads/master/db/ptmsigdb/v2.0.0/KINASE/ptm.sig.db.KINASE.flanking.human.v2.0.0.gmt", destfile = paste0(outputdir,"ptm.sig.db.KINASE.flanking.human.v2.0.0.gmt"))
```
...
res = run_ssGSEA2(gctFile,
                  ...
                  gene.set.databases = paste0(outputdir,"ptm.sig.db.KINASE.flanking.human.v2.0.0.gmt"),
                  ...
                  global.fdr = FALSE,
                  log.file = paste0(outputdir,"run.log"))
...
```{r}
library(readr)
res <- read_tsv(paste0(outputdir, "/", ptmSeaResults,"-combined.gct"), skip = 2)

comps <- str_replace(comparisons,"-",".")
fdrcomps <- paste0("fdr.pvalue.",comps)
setoverlaps <- paste0("Signature.set.overlap.percent.",comps)

filtered <- res %>% 
  filter(if_any(fdrcomps, ~.x < 0.01)) %>%
  filter(if_any(setoverlaps, ~.x >= 10)) %>%
...
for (comp in comps)
{
  ...
  label <- "LN- Recurrence vs no recurrence"
  if (comp == "Groups34.Group1"){ label <- "LN+ vs LN-"}
  plotTreatments <- plotTreatments +
   geom_point(mapping = aes(x = .data[[comp1]],y = id,size = .data[[fdrp]],fill = {{ label }}),shape = 21,alpha=0.85)
}
...
ggsave(filename = ptmSeaPlotFile,plot = plotTreatments,device = 'png',path = outputdir,units = 'in',width = 9,height = 7,dpi = 600)
```
