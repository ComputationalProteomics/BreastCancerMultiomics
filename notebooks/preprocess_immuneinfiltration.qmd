...
```{r}
dirP <- '/home/multiomics/data/Immune_infiltration/'
outputdir <- '/home/multiomics/results/immune_infiltration/'
...
write_tsv(x = epicdata,file = paste0(outputdir,'epic_aggregated.tsv'))
```
...
write_tsv(x = ciberData,file = paste0(outputdir,'cibersortx_aggregated.tsv'))
```

## Combining both datasets
```{r}
combinedImmun <- bind_rows(epicdata,ciberData)
write_tsv(x = combinedImmun,file = paste0(outputdir,'combined_immuneinfiltration.tsv'))
```
...
write_tsv(x = hisscore,file = paste0(outputdir,'hisscore_zscore.tsv'))
```
