...
```{r Output Directory}
outputdir <- paste0('/multiomics/results/figures/boxplots/')
...
```{r}
design <- list(
    proteomics = read_tsv('/multiomics/results/design_files/design_Full_noPool.tsv'),
    phosphoproteomics = read_tsv('/multiomics/results/design_files/design_phospho_noPool.tsv'),
    transcriptomics = read_tsv('/multiomics/results/design_files/design_RNA_noPool.tsv')
)
```
...
```{r}
files <- list.files(path = '/multiomics/results/',pattern = 'stats.tsv',recursive = TRUE,include.dirs = TRUE,full.names = TRUE) %>% 
...
# Lymph Node Status
```{r}
dataSurv <- read_tsv('/multiomics/results/figures/survival_analysis/DE/AllSamples_noG2/AllSamples_noG2_RFi_event/cox_analysis_res.tsv') |> 
...
dataMofa <- read_tsv('/multiomics/results/figures/survival_analysis/MOFA/AllSamples_noG2/AllSamples_noG2_LN_RFi_event/cox_analysis_res.tsv') |> 
    mutate(variable = str_replace_all(string = variable, pattern = '\\.(?=proteomics|phosphoproteomics|transcriptomics)', replacement = '_')) |> 
    filter(p.value_multivariable<0.05) |> 
    mutate(
        dataset = map(
...
```{r}
dataSurv <- read_tsv('/multiomics/results/figures/survival_analysis/DE/Group1vsGroup2/Group1vsGroup2_DRFi_event/cox_analysis_res.tsv') |> 
    mutate(variable = str_replace_all(string = variable, pattern = '\\.(?=proteomics|phosphoproteomics|transcriptomics)', replacement = '_')) |> 
    filter(p.value_multivariable<0.05) |> 
    mutate(
        dataset = map(
...
dataMofa <- read_tsv('/multiomics/results/figures/survival_analysis/MOFA/Group1vsGroup2/Group1vsGroup2_DRFi_event_DRFi_event/cox_analysis_res.tsv') |> 
    mutate(variable = str_replace_all(string = variable, pattern = '\\.(?=proteomics|phosphoproteomics|transcriptomics)', replacement = '_')) |> 
    filter(p.value_multivariable<0.05) |> 
    mutate(
        dataset = map(
