...
engine: knitr
params:
  dataPath: '/multiomics/results/consensus_clustering/DuctalvsLobular/proteome/proteome.k=6.consensusClass.csv'
...
outputdir <- '/multiomics/results/figures/survival_analysis/consensus_cluster/'
...
clusters <- map(.x = seq_len(length(cox_models_omics)),function(x){
  ...
  list_rbind() %>% 
    ...
    mutate(label=c(paste0('Cluster.',1:6,'.vs.Others')))
...
cairo_pdf(filename = paste0(outputdir,'forestplot_cluster',params$event,'.pdf'),onefile = TRUE,width = 9,height = 9)
