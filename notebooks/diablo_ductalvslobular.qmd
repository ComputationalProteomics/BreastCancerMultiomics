...
if(!dir.exists(paths = '~/multiomics/results/figures/DIABLO/DuctalvsLobular')) {
  dir.create(path = '~/multiomics/results/figures/DIABLO/DuctalvsLobular',recursive = TRUE)
}
```
...
perf.diablo <- future(seed = TRUE,globals = TRUE,packages = c('mixOmics'),gc = TRUE,
                      expr = {
                        ...
                        perf(basic.diablo.model, validation = 'Mfold',folds = 5, nrepeat = 3,auc = TRUE,cpus =parallel::detectCores()-2)
                        }
                      )
